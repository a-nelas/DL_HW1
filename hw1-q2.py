...
def mse_derivative(X, y, pred):
    return X.dot(-2).dot(y - pred)
...
def solve_analytically(X, y):
    ...
    """
    return (np.linalg.inv(X.T.dot(X) + np.identity(X.T.shape[0]).dot(lmd)).dot(X.T)).dot(y)
...
class LinearRegression(_RegressionModel):
    ...
    def update_weight(self, x_i, y_i, learning_rate=0.001):
        ...
        self.w).
        """
        w_upd = mse_derivative(x_i, y_i, self.predict(x_i))
        self.w -= w_upd.dot(learning_rate)
...
class NeuralRegression(_RegressionModel):
    ...
    def update_weight(self, x_i, y_i, learning_rate=0.001):
        ...
        db2 = -2 * (y_i - z2)
        dw2 = np.dot(db2, a1.reshape(1,-1))
        db1 = np.dot(db2, self.w2) * relu_derivative(z1)
        dw1 = np.dot(db1.reshape(-1,1), x_i.reshape(1,-1))
